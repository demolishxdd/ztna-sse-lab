...
- **Target Applications & Protocols Tested**:
  - Web Portals: GLPI (`10.240.X.X:80`) & Zabbix (`10.240.X.Y:80`)
  - Administration Protocols: SSH (`192.168.X.X:22`), RDP (`192.168.X.Y:3389`), SMB (`192.168.X.Y:445`), and TCP Agent (`192.168.X.X:10050`)
- **Roles Evaluated**:
  - `Director`: Full access to web portals, SSH, RDP, SMB, and TCP 10050.
  - `IT-Admin`: Access to GLPI web portal and SSH terminal only.
  - `HR / Staff`: Access to Zabbix web portal and TCP 10050 only.
...
- Private Anycast & Server IPs $\rightarrow$ Completely Obfuscated (`10.240.X.X`, `192.168.X.X`)
