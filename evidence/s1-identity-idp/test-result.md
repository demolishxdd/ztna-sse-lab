...
|---|---|---|---|---|
| `user.director@lab.internal` | Director | SAML + MFA (Passkey/Push) | SUCCESS | `Entra-ID-SAML-Auth` |
| `user.itadmin@lab.internal` | IT-Admin | SAML + MFA (Authenticator) | SUCCESS | `Entra-ID-SAML-Auth` |
| `user.hr@lab.internal` | HR / Staff | SAML + MFA (Authenticator) | SUCCESS | `Entra-ID-SAML-Auth` |
...
Below is an excerpt from the SCM `Network/Authentication` log confirming successful token validation and claim extraction[cite: 1]:
	...
	Authenticated User Domain: lab.internal
	Authenticated User Name: user.director
	...
	16:49:31 | Authentication Success| Session token generated for user: user.director
